...
of a neuron, a global component for non-stationarity, and additional
terms for trial-by-trial variability (G-ETMV method).
...
def fit_glm_g_etmv(
    ...
    """Fit a GLM for the G-ETMV Granger causality method.
    ...
    Returns
    -------
    beta_trials : np.ndarray
        Trial-by-trial variability coefficients, shape (nTrials,).
    ...
    beta_trials_p_vals : np.ndarray
        P-values for trial coefficients, shape (nTrials,).
    ...
    # Total duration of history in samples
    total_history_samples = history_regressor['binDuration_samples'] * n_bins_history

    # Final size of the design matrix X (with 2 extra rows for regularizers)
    dim2 = global_regressor['nBins'] + n_neurons * n_bins_history + n_trials
    dim1 = (len_trial_samples - n_bins_history * history_regressor['binDuration_samples']) * n_trials

    X_neuron = np.zeros((dim1 + 2, dim2))
    Y_neuron = np.zeros(dim1 + 2)
    ...
    for curr_trial_ind in range(n_trials):
        ...
        for curr_time_ind in range(total_history_samples, len_trial_samples):
            ...
            # Assemble row: [trial vars, global params, history params (column-major flatten)]
            X_neuron[curr_row_ind, :] = np.concatenate(
                [trial_vars, parms_global, parms_history.flatten(order='F')]
            )
            curr_row_ind += 1
    ...
    # Regularizers to help convergence
    X_neuron[dim1, :n_trials] = 1.0
    Y_neuron[dim1] = 0.0
    X_neuron[dim1 + 1, :n_trials] = 1.0
    Y_neuron[dim1 + 1] = 1.0
    ...
    # Split parameter vector into trial, global, and history components
    beta_trials = par_est[:n_trials]
    beta_trials_p_vals = par_est_p_vals[:n_trials]

    beta_global = par_est[n_trials:n_trials + global_regressor['nBins']]
    beta_global_p_vals = par_est_p_vals[n_trials:n_trials + global_regressor['nBins']]

    # Reshape history parameters: (nBinsHistory, nNeurons) column-major → (nNeurons, nBinsHistory)
    tmp = par_est[n_trials + global_regressor['nBins']:].reshape(
        n_bins_history, n_neurons, order='F'
    ).T
    tmp1 = par_est_p_vals[n_trials + global_regressor['nBins']:].reshape(
        n_bins_history, n_neurons, order='F'
    ).T
    ...
    # Flip columns (time 0 at index 0)
    beta_history = np.fliplr(tmp)
    beta_history_p_vals = np.fliplr(tmp1)

    return beta_trials, beta_global, beta_history, dev, beta_trials_p_vals, beta_global_p_vals, beta_history_p_vals
