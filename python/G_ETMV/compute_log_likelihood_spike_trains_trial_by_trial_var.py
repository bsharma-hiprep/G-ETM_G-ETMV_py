...
method G-ETMV (with trial-by-trial variability) described in:
...
def compute_log_likelihood_spike_trains_trial_by_trial_var(
    spikes,
    beta_trials,
    beta_global,
    beta_history,
    ...
    """Compute log-likelihood of spike trains for the G-ETMV model.
    ...
    for curr_trial_ind in range(n_trials):
        ...
        # Modulation for the current trial
        trial_lambda = beta_trials[curr_trial_ind]

        # Compute the effect of each neuron on spiking history
        ...
        total_lambda = global_lambda + history_lambda + trial_lambda
        P = np.exp(total_lambda) / (1.0 + np.exp(total_lambda))

        # Consider only from totalHistory_samples onward
