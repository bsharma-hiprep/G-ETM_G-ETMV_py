...
def _fit_one_etmv_round1(spike_trains, global_regressor, history_regressor,
                          neuron_ind, n_regressor_steps, hist_ind, global_ind):
    """Fit GLM for one (neuron, history-bins, global-bins) combination – Round 1 (G-ETMV).
    ...
    ll = compute_log_likelihood_spike_trains_trial_by_trial_var(
        spike_trains, tmp_trials, tmp_global, tmp_history,
        global_regressor, history_regressor, neuron_ind
    )
    aic_val = -2.0 * ll + 2.0 * (tmp_history.size + tmp_global.size + tmp_trials.size + 1)
    return (neuron_ind, hist_ind, global_ind,
            tmp_trials, tmp_global, tmp_history, dev,
            tmp_trials_p_vals, tmp_global_p_vals, tmp_history_p_vals,
            ll, aic_val)


def _fit_one_etmv_causal(spike_trains, global_regressor, history_regressor,
                           target_ind, n_history_bins, trigger_ind, baseline_dev):
    """Fit GLM for one (target, trigger) pair – causal step (G-ETMV).
    ...
    return (target_ind, trigger_ind, tmp_trials, tmp_global, tmp_history, dev_ratio)
...
def run_granger_g_etmv(spike_trains, global_regressor_in, history_regressor,
    ...
    for curr_global_regressor_ind in range(n_global_regressor_n_bins):
        ...
        for (n_ind, h_ind, g_ind,
            ...
            beta_trials[n_ind][h_ind][g_ind] = tmp_trials
            beta_trials_p_vals[n_ind][h_ind][g_ind] = tmp_trials_p_vals
    ...
    for target_neuron_ind in range(n_neurons):
        ...
        for (tgt_ind, trg_ind, tmp_trials, tmp_global, tmp_history, dev_ratio) in causal_results:
            beta_trials_causal[tgt_ind][trg_ind] = tmp_trials
